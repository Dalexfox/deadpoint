...
- **Sub-10-second logging** (the Week-1 friction batch) — sticky smart defaults (last grade + last gym from AsyncStorage, home gym fallback), grade-first section order with tappable grade labels, fixed-footer "LOG SEND" button (never below the fold), one combined photo+video picker with no forced crop (2-min video cap), non-blocking high-point check (the celebration overlays the reward screen instead of delaying it), success haptics.
