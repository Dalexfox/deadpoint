...
src/components/
  ...
  ClimbThumb.tsx       — Cover thumbnail for a climb card in the My Climbs + /user/[id] grids, the gym Current Climbs sends grid + PROBLEMS ON THE WALL cards, and the problem page's beta grid. Handles all 3 media cases so a card is NEVER blank: photo → `<Image>`; **video → a frame grabbed with `expo-video-thumbnails` (cached per URL) + a ▶ badge** (a grade placeholder shows while it generates / if it fails); no media → 🧗. ⚠️ Fixes the bug where video climbs showed no cover (a grid `<Image>` can't render a video URL). Server-side posters at upload would be the longer-term fix.
