...
src/components/
  ...
  ShareCardSheet.tsx   — Full-screen preview + share flow. Resolves the still (photo URI as-is, or a frame via `expo-video-thumbnails.getThumbnailAsync` for videos; media-less → branded), `Image.prefetch`es remote stills so the capture isn't blank, previews `ShareCard`, then on Share `captureRef` → `expo-sharing.shareAsync` (IG story/feed, Messages, Save to Photos…). Opened from the feed + session-detail share buttons via a `ShareInput`.
...
- **Shareable IG card** (Strava-style) — the feed + session-detail Share button opens `ShareCardSheet`: a branded portrait card (video still / photo + Grade / Gym / Date + Deadpoint mark) captured with `react-native-view-shot` and handed to the native share sheet (`expo-sharing`) for Instagram story/feed, Messages, or Save to Photos. Video stills via `expo-video-thumbnails`. **Needs a build** (native modules). The growth flywheel for the gym seed.
