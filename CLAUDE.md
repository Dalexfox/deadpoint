...
src/components/
  ...
  ClimbThumb.tsx       — Cover thumbnail for a climb card in the My Climbs + /user/[id] grids. Handles all 3 media cases so a card is NEVER blank: photo → `<Image>`; **video → a frame grabbed with `expo-video-thumbnails` (cached per URL) + a ▶ badge** (a grade placeholder shows while it generates / if it fails); no media → 🧗. ⚠️ Fixes the bug where video climbs showed no cover (a grid `<Image>` can't render a video URL). Server-side posters at upload would be the longer-term fix.
  VideoBackground.tsx  — Inline full-bleed video via expo-video (useVideoPlayer + VideoView). Mounted only for video posts; autoplays/loops while isActive, pauses otherwise. ⚠️ Any **FlatList of video cards** (the feed, GroupedCard pages, ClimbReel) MUST set `extraData` to include the active index (+ mute) — e.g. `extraData={\`${activeIndex}|${videoMuted}\`}`. Without it, scrolling away doesn't re-render the old cell, so its `isActive` stays true and the **video/audio keeps playing** off-screen (this was the "audio keeps playing after you scroll" bug). Takes a `muted` prop (player.muted) and a `rate` prop (player.playbackRate; default 1 — `preservesPitch` defaults true so 2× audio stays natural-pitched). Used by the feed FullScreenCard (+ group pages), session/[id], AND the send-screen video cover preview (isActive=false → shows the paused first frame). **Tap-to-mute:** a tap anywhere on a video card toggles sound; the feed holds one global `videoMuted` (TikTok-style, persists across cards), session/[id] holds its own. A speaker badge (volume-high/volume-mute Ionicon) shows the state. Default = sound ON. **Hold-to-speed (feed only):** press-and-hold a video card boosts it to 2× via the `rate` prop while held; releasing restores 1×. Implemented in the feed `FullScreenCard` — the tap layer is a `Pressable` (onPress=mute, onLongPress[delay 180ms]=`setBoosting(true)`, onPressOut=`setBoosting(false)`); a long press doesn't also fire the tap, so holding never toggles mute. A centred `2×` badge shows while held.
...
**⚡ Quick Log — now the DEFAULT** (was a shortcut). Tapping the center **Log tab opens the quick-log composer directly** (`/log-flow/send?quick=true`) — see "App Tab Bar". It skips the recognition photo, hold-color/wall, and catalog matching entirely; the detailed identify flow is an opt-in "Identify the exact climb →" link in the composer. (The gym-page "Log a Climb" CTA still goes to the gym's identify Screen 1 — a deliberate per-gym catalog-contribution context; could be aligned to fast later.) In quick mode `send.tsx` hides the problem context pill, labels the step header "Quick log", and defaults the gym to the user's `home_gym_id`. The submit needs no changes — with no `problemId`/`newProblem` params, `finalProblemId` is null, so the climb logs with `problem_id: null` (no catalog attribution → can't be conflated with another same-color/grade/wall climb) and `recompute_problem_cover` is skipped. Best for fast bulk / quiet logging; the full identify flow stays for contributing a problem to the catalog.
...
- Submit sequence — **optimistic** so a big video upload never makes "logged" wait: (1) insert problem if new (auto-name = "Blue V4 Main Wall", custom_name if entered); (2) insert session; (3) insert climb with problem_id — these are awaited (the post must exist). THEN, **in the background (fire-and-forget, survives navigation):** upload send media → update `session.media_url` → recompute `problems.media_url` (most-liked public session's media); upload the new-problem start photo; `syncHomeGymAfterSubmit`. (4) high-point query (awaited — drives the celebration); (5) show "CLIMB LOGGED" ~1.8s → navigate to feed. ⚠️ Trade-off: the post appears instantly without media; `media_url` fills in when the background upload lands (visible on the next feed load). Upload failures still `Alert` (it's a global API). The climb row is always saved synchronously — only media attachment is backgrounded.
