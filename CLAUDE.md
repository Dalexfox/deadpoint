...
src/app/
  ...
  SplashGate.tsx       — Animated "two doors" launch overlay. The WHOLE screen is the Deadpoint speckled gold-dot pattern (a jittered grid of SAND/SAND_LT rounded dots, rendered via react-native-svg) on #0d0a05, with the "D" mark drawn from brighter dots in the centre (`D_PATTERN`) — so the logo emerges FROM the speckle, no opaque image overlay. Holds briefly, then two panels each carrying their half of the full-screen pattern slide apart to reveal the app, then unmounts. The field + logo dots are generated once (memoised) and both doors render the identical SVG clipped to their half, so the seam is seamless when closed. Sits under the static native splash so there's no flash. Rendered once at root (_layout.tsx).
```
