...
src/components/
  ...
  ShareCardSheet.tsx   — Full-screen preview + share flow. **Image** post → the photo is the hero. **Video** post → samples ~6 frames across the clip (`expo-video-thumbnails.getThumbnailAsync` at fixed times; out-of-range times drop) into an Instagram-style **cover filmstrip** — tap a frame to set the card's still — PLUS a **"Share full video"** action (downloads the clip via `expo-file-system` → `expo-sharing` so it plays for the audience). **Media-less** → branded card. `Image.prefetch`es remote stills so the capture isn't blank; "Share card" does `captureRef` → `expo-sharing.shareAsync` (IG story/feed, Messages, Save to Photos…). Opened from the feed + session-detail share buttons via a `ShareInput`.
